...
namespace LINQLastExample
{
    ...
    class Program
    {
        static void Main()
        {
            ...
            Console.WriteLine(filteredEmployees[filteredEmployees.Count - 1].EmpID + ", " + filteredEmployees[filteredEmployees.Count - 1].EmpName + ", " + filteredEmployees[filteredEmployees.Count - 1].Job + ", " + filteredEmployees[filteredEmployees.Count - 1].Salary);

            //Last
            Employee lastEmployee = employees.Last(emp => emp.Job == "Manager");
            Console.WriteLine(lastEmployee.EmpID + ", " + lastEmployee.EmpName + ", " + lastEmployee.Job + ", " + lastEmployee.Salary);

            //LastOrDefault
            Employee lastEmployee2 = employees.LastOrDefault(emp => emp.Job == "Clerk");
            if (lastEmployee2 != null)
            {
                Console.WriteLine(lastEmployee2.EmpID + ", " + lastEmployee2.EmpName + ", " + lastEmployee2.Job + ", " + lastEmployee2.Salary);
            }
            else
            {
